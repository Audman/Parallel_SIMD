...
static void *mt_worker(void *arg)
{
    ...
    grayscale_scalar(th->src + th->start * 3, th->dst + th->start * 3,
                     th->end - th->start);
    return NULL;
}
...
static void grayscale_mt(const uint8_t *src, uint8_t *dst, size_t npixels, int nthreads)
{
    ...
    for (int i = 0; i < nthreads; i++)
    {
        ...
        args[i].end   = (i == nthreads - 1) ? npixels : args[i].start + chunk;
...
{
    thread_arg *th = (thread_arg *)arg;
    grayscale_simd(th->src + th->start * 3, th->dst + th->start * 3,
                   th->end - th->start);
    return NULL;
}
...
int main(int argc, char *argv[])
{
    ...
    int ok = (memcmp(out_scalar, out_simd,    nbytes) == 0) &&
             (memcmp(out_scalar, out_mt,      nbytes) == 0) &&
             (memcmp(out_scalar, out_simd_mt, nbytes) == 0);
